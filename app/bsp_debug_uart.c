...
* 备注：
*/
uint8_t uart_init(user_uart_cfg_t *cfg)
{
...
void StartDebugTask(void *argument)
{
    ...
    shell_init();
    debug_uart_init();
	printf("%s\r\n", g_build_time_str);   //2022-12-09 增加编译时间打印。
    while (1)
    {
        // 获取信号量
        if (xSemaphoreTake(s_user_debug_info.sem, (TickType_t)300) == pdTRUE)
        {
            // 如果buf不为空
